...
const MDomandaForm = ({ key, initialValue, parentValue, arrayManager, onSubFormChange }) => {
  const { setFieldValue } = useFormikContext();
  const [value, setValue] = React.useState(initialValue);
  const [expanded, setExpanded] = React.useState(toNumberOr(value && value.tipo, 0) === 0 || false);
  ...
  const moveRisposta = (op, arrayHelper, index) => {
    op === 'movedown'
      ? index < value.risposte.length - 1 && arrayHelper.move(index, index + 1)
      : op === 'moveup' && index > 0 && arrayHelper.move(index, index - 1);
  };

  const arrayManagerRisposte = (arrayHelper, index) => op => {
    return op === 'delete'
      ? index === 0
  ...
  const getRatingVals = key =>
    value && value.risposte && value.risposte[0] && value.risposte[0][key] && value.risposte[0][key];

  const setOptionsTipoUnico = (replace, val, index) =>
    setFieldValue(
      'risposte',
      value.risposte.map((ris, iRis) => {
        const risNes = { ...ris };
  ...
  const onClickOption = (replace, index) => (field, val) => {
    value.tipo === TipoQuestionName.unica
      ? setOptionsTipoUnico(replace, val, index)
      : replace(index, { ...value.risposte[index], val });
    setFieldValue('._tmp', index);
  };

  const clonaDomanda = () => arrayManager('clone', value);

  const renderScala = () => (
    <Box component="fieldset" mb={3} borderColor="transparent">
      <GridChilds view={[12]}>
        {value.tipo === TipoQuestionName.scala && (
  ...
  const renderAddRisposta = ({ arrayHelper }) =>
    value.tipo !== TipoQuestionName.aperta &&
    value.tipo !== TipoQuestionName.scala && (
      <Box
          ...
          <>
            ...
            >
                  ...
                  <GridChilds view={[8, 2, 2]} style={{ alignItems: 'center' }} width="100%">
                    ...
                    {parentValue && (
                      <GridChilds spacing={1} view={[6, 6]} style={{ alignItems: 'center', marginLeft: '12px' }}>
                        ...
                        <Box>
                          <Button onClick={() => arrayManager('delete')}>
              ...
              <AccordionDetails style={{ flexDirection: 'column' }}>
                <>
                  {value.risposte && value.tipo !== TipoQuestionName.titolo && value.tipo !== TipoQuestionName.aperta && (
                    <ToFieldArray
                      name={'risposte'}
                      renderMaxElem={
                        value.tipo === TipoQuestionName.aperta || value.tipo === TipoQuestionName.scala ? 1 : 0
                      }
                      values={value.risposte && value.risposte[0] ? value.risposte : [{ risposta: '' }]}
...
const MDomandaForm = ({ name, fieldProps, setFieldValue }) => {
  ...
  values: valueTmp && valueTmp.domande && valueTmp.domande[index]
